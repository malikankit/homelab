#!/usr/bin/env bash
# ufw firewall rules for mba13-linux (mba13, Asahi Linux) — Tailnet +
...
#
# This reflects the rules already live on mba13-linux (captured in
# ufw_status.md) plus geekom-linux (geekom), which was pending. The 41641/udp rule
...
# --- Known AM-tailnet peers allowed to SSH into mba13-linux ---
GEEKOM_LINUX_IP="100.78.110.5"                # geekom-linux (geekom)
MBP16_MAC_AM_IP="100.87.74.44"       # mbp16-mac (mbp16), AM tailnet
MBP16_MAC_ZENITH_IP="100.108.204.52" # mbp16-mac (mbp16), Zenith tailnet
...
ufw delete allow from "$MBP16_MAC_AM_IP" to any port 22 2>/dev/null || true
ufw delete allow from "$MBP16_MAC_ZENITH_IP" to any port 22 2>/dev/null || true

# --- SSH: only from known peers, arriving via the tailscale0 interface ---
ufw allow in on tailscale0 from "$GEEKOM_LINUX_IP" to any port 22 proto tcp comment 'SSH from geekom-linux'
ufw allow in on tailscale0 from "$MBP16_MAC_AM_IP" to any port 22 proto tcp comment 'SSH from mbp16-mac (AM tailnet)'
ufw allow in on tailscale0 from "$MBP16_MAC_ZENITH_IP" to any port 22 proto tcp comment 'SSH from mbp16-mac (Zenith tailnet)'
